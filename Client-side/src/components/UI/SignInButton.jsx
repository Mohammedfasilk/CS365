 ...
 const handleLogin = async () => {
  try {
    ...
    if (userExist.success) {
      ...
      // await instance.logoutPopup({ postLogoutRedirectUri: "/" });
      // instance.getAllAccounts().forEach((acc) => instance.removeAccount(acc));

      // // Retry login with forced account picker
      // const retryLogin = await instance.loginPopup({
      //   ...loginRequest,
      //   prompt: "select_account",
      // });

      // const retryAccount = retryLogin.account;

      // if (!retryAccount) {
      //   console.warn("Retry login failed.");
      //   return;
      // }
      ...
      //   `${import.meta.env.VITE_CS365_URI}/api/auth`,
      //   { current_user: retryAccount.username }
      // );

      // if (retryRes.data) {
      //   navigate("/home");
      // } else {
      //   alert("Still not found. Please contact your administrator.");
      // }
    }
  } catch (error) {
